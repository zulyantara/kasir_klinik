...
class Satuan_barang extends CI_Controller
{
    ...
    function ajax_grid()
    {
		...
		if( ! empty($requestData['search']['value']) )
		{
			...
			$where = "(sb_ket LIKE '%".$requestData['search']['value']."%')";
        ...
        if( ! empty($res))
        {
    		...
    		{
    			$random = rand();
    			$id = base64_encode($random."-".$row->sb_id);
    			$edit = base64_encode($random."-edit");
    			$delete = base64_encode($random."-delete");

    			$nestedData = array();
    			$nestedData[] = strtoupper(trim($row->sb_ket));
    			$nestedData[] = "
    ...
    function form()
	{
        ...
        else
        {
    		...
    		$this->form_validation->set_rules('txt_sb_ket', 'Satuan Barang', 'required');
    		...
    		else
    		{
                ...
                $sb_ket = strtoupper($this->input->post("txt_sb_ket"));
    			if($this->input->post("btn_simpan") === "btn_simpan")
    			{
                    $field[] = "sb_ket,";
    				$field[] = "sb_insert_date,";
    				$field[] = "sb_insert_user";
    			...
    			elseif ($this->input->post("btn_simpan") === "btn_ubah")
    			{
    				...
    				$sb_id = "'".$this->input->post("txt_sb_id")."'";
                    $data["sb_ket"] = "'".trim($sb_ket)."',";
    				$data["sb_update_date"] = "'".date("Y-m-d H:i:s")."',";
                    $data["sb_update_user"] = $user_id;

    				// kirim data
    				$update_data = $this->mm->update_data($this->table, $this->pk, $sb_id, $data);
    				if($update_data === 1)
    				{
    					$rand = rand();
