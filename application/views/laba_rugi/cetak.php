    ...
    <tbody>
        ...
        foreach ($qry_ja as $row_ja)
        {
            ...
            foreach ($qry_ka->result() as $row_ka)
            {
                ...
                $sql_total .= " UNION ";
                $sql_total .= "SELECT SUM(pd_qty*pd_harga_beli) as harga FROM v_pembelian WHERE MONTH(pd_insert_date) = ".$this->db->escape($bulan)." AND ka_kode='".$row_ka->ka_kode."' GROUP BY MONTH(pd_insert_date)";
                // echo $sql_total;
